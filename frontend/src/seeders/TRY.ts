...
const stripe = new Stripe(
  "sk_test_51Nl45NJI6EvYO1AiOPoOxpA52rFWdqKdEGMkv5AlcCmcdfhvmB3WK7qkNysumjucIrzuRFhyfBuKHQ3EE2yMj8KJ00zxtkcqEN"
);

/**
 * The API to get all the datas(products) from the database should be mentioned here
 */
const fetchProductsURL = "http://localhost:4000/products";

/**
 * The default image when there is no image string present in the application
 */
const defaultImageIfnoImage =
  "https://images.unsplash.com/photo-1546760888-ef9d7caee133?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=1887&q=80";
 ...
 */
const getProductPutUrl = (id): string => "http://localhost:4000/products/" + id;
...
const seedProducttoStripe = async () => {
  try {
    ...
    for (const productData of products) {
      ...
      await axios.put(getProductPutUrl(productData.id), {
        ...productData,
        priceId: price.id,
      });
      console.log(
        `Product "${productData.name}" created with price id ${price.id}`
      );
    }
  } catch (e) {
